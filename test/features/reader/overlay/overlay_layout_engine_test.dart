...
void main() {
  ...
  test('builds deterministic element geometry and highlighted tokens', () {
    ...
    expect(tokens[0].style.backgroundColor, const Color.fromRGBO(255, 213, 79, 0.85));

    expect(tokens[1].raw, 'world');
    expect(tokens[1].globalWordIndex, 1);
    expect(tokens[1].style.backgroundColor, const Color.fromRGBO(212, 237, 188, 0.8));
  });

  test('keeps whitespace tokens so rich text spacing remains stable', () {
    const overlay = TextOverlayConfig(
      version: 1,
      elements: [
        TextElement(
