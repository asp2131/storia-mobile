#!/usr/bin/env bash
# pi-symphony.sh — Pi-native Linear orchestrator (Symphony-shaped, no Codex).
#
# Polls Linear for tickets in `Todo`, `Rework`, or stale `In Progress` (resume),
# dispatches `pi chain storia-build-feature` in an isolated git worktree, runs
# ./bin/verify.sh, opens a PR, moves the ticket to `Human Review`. Reads
# WORKFLOW.md status map.
#
# Per-state behavior:
#   Todo        → fresh worktree off origin/main, run pi chain, open PR
#   Rework      → fetch existing PR feedback via gh, prepend to pi prompt
#   In Progress → resume only if local worktree exists (i.e. we crashed mid-run);
#                 don't grab tickets a human is driving manually
#
# Usage:
#   ./bin/pi-symphony.sh             # foreground loop
#   ./bin/pi-symphony.sh --once      # process current Todo tickets, exit
#   ./bin/pi-symphony.sh --dry-run   # show what would dispatch, don't act
#
# Required env:
...
load_state_ids() {
  ...
  if [ "$(echo "$resp" | jq -r '.data.project // empty')" = "" ]; then
    echo "[pi-symphony] could not resolve project $PROJECT_SLUG" >&2
    echo "$resp" | jq . >&2
    exit 1
  fi
  echo "$resp" | jq '{
...
### Validation
- [ ] \`./bin/verify.sh\` — Flutter analyze + tests
- [ ] Playwright WebM proof when UI/browser-verifiable — use \`app-review@storia.kids\` review bypass and record reaching \`/library\`
...
Playwright proof requirement for UI/browser-verifiable work:
- If the change touches UI or a browser-verifiable flow, create a WebM proof under recordings/ before stopping.
- Canonical app access path: Intro → Start your journey → enter app-review@storia.kids → complete parent birth year → complete onboarding → reach /library → smoke the changed feature.
- Use: flutter run -d chrome, playwright-cli open <url>, playwright-cli video-start, perform the flow, playwright-cli video-stop --filename=recordings/${ident}-proof.webm, playwright-cli close."
  ...
  local proof_md proof_tmp proof_status
  proof_md=""
  proof_status=0
  (cd "$wt" && symphony_requires_playwright_proof "$title" "$desc" "$GIT_REMOTE/$GIT_BASE") || proof_status=$?
  if [ "$proof_status" = "0" ]; then
    ...
    if (cd "$wt" && symphony_build_playwright_proof_markdown "$ident") >"$proof_tmp" 2>&1; then
      ...
      local proof_error
      proof_error="$(cat "$proof_tmp")"
      rm -f "$proof_tmp"
      finalize_blocker "$id" "$ident" "Playwright video proof is required before PR handoff but evidence is missing or invalid. ${proof_error}"
      return 1
    fi
  elif [ "$proof_status" = "1" ]; then
    proof_md="Playwright video proof: not required by policy (PLAYWRIGHT_PROOF_MODE=$(symphony_playwright_proof_mode))."
  else
    finalize_blocker "$id" "$ident" "Playwright proof policy configuration failed. Check PLAYWRIGHT_PROOF_MODE=$(symphony_playwright_proof_mode)."
    return 1
  fi
